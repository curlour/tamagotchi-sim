...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/mangakat.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/mangaka.png").convert()

    desk = pygame.image.load("Sprites/Misc/bg/mangakad.png").convert()

    blans = pygame.Surface((40, 28)).convert()
    blans.fill((255, 255, 255))
    ...
    while kr:
        if tuts:
            ...
            screen.blit(playbk, [0, 0])
            if end == 0:
                spr = 3 + ((anifr % 12 > 5) * [1, 6][tooln == 1])
            else:
                spr = 7
            screen.blit(asprs[avars[3][5]][spr], [104, 80])
            screen.blit(desk, [0, 0])
            if tooln < 2:
                screen.blit(blans, [100, 107])
        else:
            screen.blit(playbk, [0, 0])
            if end == 3:
