...
def game(avars, asprs, screen):
    ...
    pcl = [randint(0, 11), randint(0, 11)]
    ccl = [(((((pcl[0] // 2) + 3) % 6) * 2) + ((pcl[0] % 2) == 0)),
           (((((pcl[1] // 2) + 3) % 6) * 2) + ((pcl[1] % 2) == 0))]
    ycl = [-1, -1]
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/fashiont.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/fashion.png").convert()

    clth = []
    c = pygame.image.load("Sprites/Misc/obj/clothes.png")
    ...
    for i in range(12):
        c.set_palette(((0, 255, 255), (0, 0, 100), p[1][i // 2], p[2][i // 2], p[0][i // 2]))
    ...
    while kr:
        if tuts:
            ...
            if end == 0:
                ...
                spr = 5 + (2 * (ycl != ccl))
            for i in range(12):
                screen.blit(clth[i], [(28 + (32 * (i // 2))), (32 * (1 + (i % 2)))])
            for i in [0, 1]:
                screen.blit(clth[pcl[i]], [(2 + (212 * i)), 100])
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                if pb in [1, 3] and (24 < mp[0] < 216) and (28 < mp[1] < 92) and play and end == 0:
                    ycl[pb // 2] = (2 * ((mp[0] - 24) // 32)) + (mp[1] > 60)
                    if ycl.count(-1) == 0:
                        end = 1
                        anifr = 83
                        pygame.mixer.stop()
                        sound[9 + (3 * (ycl != ccl))].play()
                    else:
                        pygame.mixer.stop()
                        sound[2].play()
        ...
        if anifr < (23 + (72 * play)):
            ...
            if strt:
                ...
                if scr < 12 or avars[avars[3][5]][15] != 299 or avars[avars[3][5]][6] < 667 or avars[avars[3][5]][10] < 334:
                    if scr < 4:
                        pygame.mixer.stop()
                        sound[14].play()
                    else:
