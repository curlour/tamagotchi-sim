...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/buildert.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/builder.png").convert()

    pesa = pygame.image.load("Sprites/Misc/obj/buildo.png").convert()
    ...
    while kr:
        if tuts:
            ...
            screen.blit(playbk, [0, 0])
            spr = [3, 6, 8, 5][power // 24]
            screen.blit(asprs[avars[3][5]][spr], [104, 98])
            screen.blit(pesa, [72, (98 - (8 * (power // 18)))])
        else:
            screen.blit(playbk, [0, 0])
            if end == 3:
                ...
                screen.blit(pesa, [72, (98 - (8 * (anifr // 6)))])
        ...
        if chngsts:
            ...
            chngsts = False
        if anifr < (23 + (264 * (play and end == 0))):
            anifr += 1
        else:
            anifr = 0
            if strt:
                ...
                if scr < 12 or avars[avars[3][5]][15] != 349 or avars[avars[3][5]][10] < 1000:
                    if scr < 4:
                        pygame.mixer.stop()
                        sound[14].play()
                    else:
