...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/enkat.png").convert()
    ...
    cncth = []
    c = pygame.image.load("Sprites/Misc/bg/enka.png").convert()
    for i in [0, 1]:
        h = pygame.Surface((240, 160)).convert()
        h.blit(c, [-(240 * i), 0])
        cncth.append(h)

    dark = pygame.Surface((240, 160)).convert()
    dark.fill((0, 0, 100))
    ...
    seqi = fnt.render(seqt, 1, (0, 153, 128))
    ...
    while kr:
        if tuts:
            ...
            if curr > -1:
                screen.blit(cncth[curr], [0, 0])
                if end == 0:
                    spr = [12, 15, 10, 5][(2 * curr) + (anifr % 12 > 5)]
                else:
                    spr = 7
                screen.blit(asprs[avars[3][5]][spr], [104, 99])
            else:
            ...
            if end == 3:
                ...
                screen.blit(cncth[anifr % 12 > 5], [0, 0])
                if anifr < 6:
                    screen.blit(asprs[avars[3][5]][5], [104, 99])
                else:
                    screen.blit(poof, [104, 99])
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                if pb in [1 , 3] and (24 < mp[1] < 136) and play:
                    pres = pb // 2
        ...
        if anifr < 23:
            ...
            if strt:
                ...
                if scr < 12 or avars[avars[3][5]][15] != 306 or avars[avars[3][5]][9] < 1000:
                    if scr < 4:
                        pygame.mixer.stop()
                        sound[14].play()
                    else:
