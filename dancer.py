...
def game(avars, asprs, screen):
    ...
    seq = [randint(0, 3), randint(0, 3), randint(0, 3)]
    turn = 0
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/dancert.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/dancer.png").convert()

    pals = chsprs(choice([283, 284, 305, 306]))
    ...
    while kr:
        if tuts:
            ...
            screen.blit(playbk, [0, 0])
            if end == 0:
                screen.blit(pygame.transform.flip(pals[[10, 12, 15, 5][seq[turn]]], (seq[turn] == 2), 0), [(32 + (72 * ((seq[turn] != 1) + (seq[turn] == 2)))), (98 - (32 * (seq[turn] == 3)))])
            elif end == 1:
                screen.blit(pygame.transform.flip(asprs[avars[3][5]][[10, 12, 15, 5][seq[turn]]], (seq[turn] == 2), 0), [(32 + (72 * ((seq[turn] != 1) + (seq[turn] == 2)))), (98 - (32 * (seq[turn] == 3)))])
            else:
                screen.blit(asprs[avars[3][5]][7], [104, 98])
        else:
            screen.blit(playbk, [0, 0])
            if end == 3:
            ...
            if strt:
                ...
                if end < 2:
                    ...
                    if end == 1:
                        ...
                        d = (screen.get_size()[0] // 240)
                        p = ((p[0] // d), (p[1] // d))
                        t = (2 * ((2 * p[0]) > (3 * p[1]))) + ((480 - (2 * p[0])) > (3 * p[1]))
                        if t != seq[turn]:
                            end = 2
                            pygame.mixer.stop()
                            sound[12].play()
