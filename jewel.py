...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/jewelt.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/jewel.png").convert()

    out = pygame.Surface((240, 160)).convert()
    f = pygame.image.load("Sprites/Misc/bg/job_1.png").convert()
    out.blit(f, [-480, -640])

    pearls = []
    p = pygame.image.load("Sprites/Misc/obj/pearl.png")
    for i in range(12):
        j = pygame.Surface((16, 16)).convert()
        j.fill((0, 255, 255))
        p.set_palette_at(3, ([204, 224, 240, 255, 176, 204, 240, 255][i % 8],
                             [240, 224, 204, 204, 255, 240, 224, 204, 255, 255, 255, 240][i],
                             [255, 255, 255, 240, 255, 240, 224, 204, 240, 224, 204, 204][i]))
        p.set_palette_at(4, ([153, 176, 204, 240, 128, 153, 204, 240][i % 8],
                             [204, 176, 153, 153, 224, 204, 173, 153, 240, 240, 240, 204][i],
                             [240, 240, 240, 204, 224, 204, 224, 204, 204, 176, 153, 153][i]))
        j.blit(p, [0, 0])
    ...
    while kr:
        if tuts:
            ...
            for i in range(len(peli)):
                screen.blit(pearls[peli[i]], [(200 + (16 * (i % 2))), (96 + (16 * (i // 2)))])
            for i in range(len(seq)):
                screen.blit(pearls[seq[i]], [(24 + (16 * i)), 40])
            screen.blit(asprs[avars[3][5]][spr], [8, 96])
        else:
            screen.blit(out, [0, 0])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                    if (80 < mp[1] < 128) and (88 < mp[0] < 152) and play and end == 0:
                        seq.append((4 * ((mp[1] - 80) // 16)) + ((mp[0] - 88) // 16))
                        pygame.mixer.stop()
                        sound[2].play()
                        if len(seq) == 12:
                            corr = 1
