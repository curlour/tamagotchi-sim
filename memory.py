...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/memoryt.png").convert()

    gamebk = pygame.image.load("Sprites/Misc/bg/gamecenter.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/memory.png").convert()

    qspr = pygame.image.load("Sprites/Misc/bg/memoryq.png").convert()

    dias = []
    p = [(240, 255, 255, 255), (128, 204, 204, 255), (128, 128, 255, 255), (128, 255, 128, 255), (255, 128, 128, 255)]
    for i in range(5):
        s = pygame.image.load("Sprites/Misc/bg/memoryd.png")
        s.set_palette_at(2, p[i])
        s.convert()
        dias.append(s)
    ...
    while kr:
        if tuts:
            ...
            if end < 2:
                ...
                spr = 5 + (dlst[cdia[scr * 2]] != dlst[cdia[(scr * 2) + 1]])
            for i in range(25):
                if end == 0 or (i in cdia):
                    screen.blit(dias[dlst[i]], [(64 + (24 * (i % 5))), (24 + (24 * (i // 5)))])
                else:
                    screen.blit(qspr, [(64 + (24 * (i % 5))), (24 + (24 * (i // 5)))])
            screen.blit((fnt.render((str(lvs)), 1, (0, 0, 100))), [32, 74])
            screen.blit(asprs[avars[3][5]][spr], [(196 + ((32 - asprs[avars[3][5]][spr].get_width()) // 2)), (64 + ((32 - asprs[avars[3][5]][spr].get_height()) // 2))])
        else:
            screen.blit(gamebk, [0, 0])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                    if (24 < mp[1] < 136) and (64 < mp[0] < 176) and (mp[1] % 24 < 16) and ((mp[0] - 64) % 24 < 16) \
                       and play and end == 1:
                        n = (5 * ((mp[1] - 24) // 24)) + ((mp[0] - 64) // 24)
                        if n not in cdia:
                            cdia.append(n)
                            if len(cdia) == (2 * (scr + 1)):
                                end = 2
                                anifr = 11
                                pygame.mixer.stop()
                                sound[9 + (3 *(dlst[cdia[scr * 2]] != dlst[cdia[(scr * 2) + 1]]))].play()
        ...
        if anifr < 23:
            ...
            if strt:
                ...
                if end == 0:
                    ...
                    if dlst[cdia[scr * 2]] != dlst[cdia[(scr * 2) + 1]]:
                        lvs -= 1
                        cdia = cdia[:(2 * scr)]
                        end = 1
                        if lvs == 0:
