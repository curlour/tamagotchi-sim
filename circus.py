...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/circust.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/circus.png").convert()

    net = pygame.image.load("Sprites/Misc/bg/circusn.png").convert()

    balloon = [pygame.image.load("Sprites/Misc/item/balloon.png").convert(),
               pygame.image.load("Sprites/Misc/item/balloon1.png").convert(),
               pygame.image.load("Sprites/Misc/item/balloon2.png").convert(),
               pygame.image.load("Sprites/Misc/item/balloon3.png").convert()]

    pie = []
    p = pygame.image.load("Sprites/Misc/obj/pie.png").convert()
    for i in [0, 1]:
    ...
    while kr:
        if tuts:
            ...
            if end == 0:
                screen.blit(net, [0, 112])
                if anifr < 12:
                    screen.blit(balloon[1 + (anifr > 5)], [80, 98])
                    spr = [11, 15][anifr > 5]
                    y = 98
                else:
                    screen.blit(balloon[0], [108, [48, 16][anifr > 17]])
                    spr = 5
                    y = [64, 32][anifr > 17]
            else:
                y = 32 * (height + 1)
                if anifr < 12:
                    spr = 3
                    screen.blit(pygame.transform.flip(pie[0], 0, (anifr > 5)), [(34 + (32 * (anifr > 5))), (32 + (32 * pieh))])
                else:
                    spr = 5 + (4 * (height != pieh))
                screen.blit(balloon[3 * (anifr > 11 and (height != pieh))], [108, (y - 16)])
            screen.blit(asprs[avars[3][5]][spr], [104, y])
            if (end == 1) and (anifr > 11):
                screen.blit(pie[1], [108, (32 + (32 * pieh))])
        else:
            screen.blit(playbk, [0, 0])
            screen.blit(net, [0, 112])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
            if event.type == MOUSEMOTION and play and end == 1 and anifr < 12:
                mp = event.pos
                d = (screen.get_size()[0] // 240)
                mp = ((mp[0] // d), (mp[1] // d))
                if 32 < mp[1] < 128:
        ...
        if anifr < 23:
            ...
            if strt:
                play = True
                strt = False
                if end == 1 and anifr == 12:
                    pygame.mixer.stop()
                    sound[9 + (3 * (height != pieh))].play()
            elif play:
                if end == 0:
                    end = 1
                else:
                    ...
                    if (height != pieh) or scr == 24:
                        ...
                        avars[avars[3][5]][6] += (scr // 3)
                    pieh = randint(0, 2)
            elif end == 3:
                if scr < 24 or avars[avars[3][5]][15] != 224 or avars[avars[3][5]][8] < 667 or avars[avars[3][5]][6] < 334:
                    if scr < 8:
                        pygame.mixer.stop()
                        sound[14].play()
                    else:
