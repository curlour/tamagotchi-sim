...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/carpentert.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/carpenter.png").convert()

    butt = []
    for i in [0, 1]:
        j = pygame.Surface((16, 16))
        j.blit(playbk, [-(16 * i), 0])
    ...
    table = []
    for i in [0, 1]:
        j = pygame.Surface((64, 24))
        j.blit(playbk, [-(32 + (64 * i)), 0])
        j.set_colorkey((0, 255, 255))
        j.convert()
        table.append(j)
    table.append(pygame.image.load("Sprites/Misc/obj/table.png").convert())
    ...
    while kr:
        if tuts:
            ...
            if end == 0:
                for i in range(4):
                    screen.blit(butt[seq[i]], [(88 + (16 * i)), 24])
                spr = 6
            elif end == 1:
                spr = 11 + (anifr % 12 > 5)
            else:
                spr = 5 + (2 * (seq != ans))
            screen.blit(table[(end == 2) + (seq == ans)], [88, (104 - (8 * (seq == ans)))])
            screen.blit(pygame.transform.flip(asprs[avars[3][5]][spr], 1, 0), [56, 98])
        else:
            screen.blit(playbk, [0, 0])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                if (24 < mp[1] < 136) and play and end == 1 and pb in [1, 3]:
                    pygame.mixer.stop()
                    sound[2].play()
                    ans.append(pb // 2)
                    if len(ans) == 4:
