...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/tonest.png").convert()

    gamebk = pygame.image.load("Sprites/Misc/bg/gamecenter.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/tonesbg.png").convert()
    ...
    rec = pygame.image.load("Sprites/Misc/bg/trec.png").convert()
    ...
    while kr:
        if tuts:
            ...
            screen.blit((fnt.render((str(scr)), 1, (0, 0, 100))), [48, 68])
            if gmprt == 0:
                spr = 3
                if anifr in [5, 11, 17, 23]:
                    pygame.mixer.stop()
                    sound[(19 + ([5, 11, 17, 23].index(anifr)))].play()
            elif gmprt == 1:
                spr = 3
                if anifr in [5, 11, 17, 23]:
                    a = randint(0, 3)
                    sec.append(a)
                    pygame.mixer.stop()
                    sound[(19 + (sec[len(sec) - 1]))].play()
                if anifr > 4:
                    screen.blit(rec, [100, 56])
            elif gmprt == 2:
                if prsb == 4:
                    spr = 11
                else:
                    spr = 12
                    if prsb == sec[0]:
                        screen.blit(dc, [100, 56])
                    else:
                        screen.blit(dw, [100, 56])
            screen.blit(asprs[avars[3][5]][spr], [112, (92 + (32 - asprs[avars[3][5]][spr].get_height()))])
        else:
            screen.blit(gamebk, [0, 0])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                if 24 < mp[1] < 136 and gmprt == 2 and prsb == 4 and (pb != 2 and pb < 6):
                    anifr = 19
