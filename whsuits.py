...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/whsuitst.png").convert()
    ...
    var = pygame.image.load("Sprites/Misc/txtbox/arrow2.png").convert()
    ...
    while kr:
        if tuts:
            ...
            if gmprt == 0:
                spr = 3
                screen.blit(mcbi, [44, 72])
                screen.blit(pygame.transform.flip(mcbi, 1, 0), [92, 72])
                screen.blit(msbi, [140, 72])
                screen.blit(pygame.transform.flip(msbi, 0, 1), [188, 72])
                if anifr < 16:
                    if anifr > 7:
                        shuffle(slst)
                    screen.blit(slst[0], [36, 28])
                    screen.blit(slst[1], [84, 28])
                    screen.blit(slst[2], [132, 28])
                    screen.blit(slst[3], [180, 28])
                else:
                    screen.blit(slst[0], [36, 36])
                    screen.blit(slst[1], [84, 36])
                    screen.blit(slst[2], [132, 36])
                    screen.blit(slst[3], [180, 36])
            elif gmprt == 1:
                spr = 3
                ...
                screen.blit(var, [(44+ (48 * arp)), 88])
            else:
                if slst[prsb] == [spade, heart, club, dia][arp]:
                    spr = 5
                else:
                    spr = 7
                screen.blit(sspade, [36, 60])
                ...
                screen.blit(sclub, [132, 60])
                screen.blit(sdia, [180, 60])
                screen.blit(slst[prsb], [(36+ (48 * arp)), 36])
            screen.blit(asprs[avars[3][5]][spr], [(104 + ((32 - asprs[avars[3][5]][spr].get_width()) // 2)), (98 + (32 - asprs[avars[3][5]][spr].get_height()))])
        else:
            screen.blit(gamebk, [0, 0])
            if end == 3:
