...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/librt.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/libr.png").convert()

    bksl = []
    for i in [0, 1, 2]:
        s = pygame.image.load("Sprites/Misc/item/book.png")
        s.set_palette_at(6, [(80, 80, 255), (80, 176, 176), (80, 255, 80)][i])
        s.set_palette_at(7, [(0, 0, 224), (32, 128, 128), (0, 244, 0)][i])
        s.set_palette_at(8, [(0, 0, 153), (0, 80, 80), (0, 153, 0)][i])
        s.convert()
        bksl.append(s)
    ...
    while kr:
        if tuts:
            ...
            if end in [0, 2]:
                for i in [0, 1, 2]:
                    screen.blit(bksl[resor[i]], [(16 + (24 * i)), 101])
            if end == 0:
                for i in [0, 1, 2]:
                    screen.blit(bksl[bkor[i]], [0, (21 + (24 * i))])
            elif end == 1:
                for i in [0, 1, 2]:
                    screen.blit(bksl[bkor[i]], [(24 + (12 * (anifr // 3))), (21 + (24 * i))])
            else:
                for i in [0, 1, 2]:
                    screen.blit(bksl[bkor[i]], [216, (21 + (24 * i))])
                    screen.blit(bksl[bkor[i]], [(152 + (24 * i)), 101])
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                    if (24 < mp[1] < 96) and play and end == 1:
                        pygame.mixer.stop()
                        sound[2].play()
                        a = (mp[1] - 24) // 24
                        cbk.append(a)
                        if len(cbk) == 2:
                            bkor[cbk[0]], bkor[cbk[1]] = bkor[cbk[1]], bkor[cbk[0]]
                            cbk = []
            ...
            if strt:
                ...
                if end < 2:
                    ...
                    if end == 1:
                        ...
                        sound[9 + (3 * (bkor != resor))].play()
                else:
                    if bkor == resor:
                        scr += 1
                        end = 0
                    if bkor != resor or scr == 12:
                        end = 3
                        play = False
                ...
                if scr < 12 or avars[avars[3][5]][15] != 248 or avars[avars[3][5]][5] < 667 or avars[avars[3][5]][9] < 334:
                    if scr < 4:
                        pygame.mixer.stop()
                        sound[14].play()
                    else:
