...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/firet.png").convert()

    playbk = pygame.Surface((240, 160)).convert()
    f = pygame.image.load("Sprites/Misc/bg/job_1.png").convert()
    playbk.blit(f, [-720, -800])

    fireb = pygame.image.load("Sprites/Misc/bg/fire.png").convert()

    window = []
    for i in [0, 1]:
        j = pygame.Surface((14, 22)).convert()
        j.blit(fireb, [-(16 * i), 0])
        window.append(j)

    owo = []
    for i in [0, 1, 2]:
        j = pygame.Surface((8, 16)).convert()
        j.blit(fireb, [-(32 + (8 * i)), 0])
        owo.append(j)
    ...
    while kr:
        if tuts:
            ...
            for i in range(9):
                if firew[i] == 2:
                    screen.blit(window[0], [(9 + (24 * (i % 3))), (37 + (32 * (i // 3)))])
                if firew[i] > 2:
                    screen.blit(window[1], [(9 + (24 * (i % 3))), (37 + (32 * (i // 3)))])
            screen.blit(owo[pos], [120, 80])
            screen.blit(asprs[avars[3][5]][spr], [136, 62])
        else:
            screen.blit(playbk, [0, 0])
            if end == 3:
        ...
        if play and end == 0:
            ...
            c = c[:4]
            for i in range(9):
                if (i in c) and firew[i] < 2 and anifr % 6 == 0:
                    firew[i] += 1
                if firew[i] > 2:
                    firew[i] = (firew[i] + 1) % 10
            if firew.count(2) > 6 or scr == 12:
                firew = [[0, 0, 0, 0, 0, 0, 0, 0, 0],
                         [2, 2, 2, 2, 2, 2, 2, 2, 2]][firew.count(2) > 6]
                end = 1
                anifr = 12
                pygame.mixer.stop()
                sound[9 + (3 * (firew[0] == 2))].play()
