...
def game(avars, asprs, screen):
    ...
    req = [randint(0, 1), randint(0, 1), randint(0, 1), randint(0, 1)]
    giv = [-1, -1, -1, -1]
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/kclasst.png").convert()

    classbg = pygame.image.load("Sprites/Misc/bg/kndclass.png").convert()

    stsprs = [chsprs(randint(49, 132)), chsprs(randint(49, 132)), chsprs(randint(49, 132)), chsprs(randint(49, 132))]

    obsp = [[], []]
    o = pygame.image.load("Sprites/Misc/obj/ktob.png").convert()
    for i in [0, 1, 2, 3]:
        j = pygame.Surface((24, 24)).convert()
        j.fill((0, 255, 255))
        j.blit(o, [-(24 * (i % 2)), -(24 * (i > 1))])
    ...
    while kr:
        if tuts:
            ...
            if end == 0:
                ...
                for i in [0, 1, 2, 3]:
                    j = 3 + ((anifr % 12 > 5) == (i % 2))
                    screen.blit(stsprs[i][j], [(36 + (48 * i)), 106])
                    screen.blit(obsp[req[i]][req[i] == giv[i]], [(12 + (48 * i)), 106])
            else:
                spr = 5 + (2 * (req != giv))
                for i in [0, 1, 2, 3]:
                    j = 5 + (2 * (req[i] != giv[i]))
                    screen.blit(stsprs[i][j], [(36 + (48 * i)), 106])
                    screen.blit(obsp[req[i]][req[i] == giv[i]], [(12 + (48 * i)), 106])
            screen.blit(asprs[avars[3][5]][spr], [180, 72])
        else:
            screen.blit(classbg, [0, 0])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                if pb in [1, 3] and (24 < mp[1] < 136) and (12 < mp[0] < 204) and play and end == 0:
                    pygame.mixer.stop()
                    sound[2].play()
                    i = (mp[0] - 12) // 48
                    giv[i] = (pb // 2)
                    if (giv[i] != req[i]) or (giv.count(-1) == 0):
                        anifr = 95
