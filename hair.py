    ...
    while kr:
        if tuts:
            ...
            if end == 0:
                ...
                cs = 16
                if cuo > -1:
                    screen.blit(hobj[cuo][anifr % 12 > 5], [(184 + (8 * (anifr % 12 > 5))) ,72])
            else:
                spr = 7 - (2 * (hlen == cln == flen))
                cs = 8 - (3 * (hlen == cln == flen))
            screen.blit(ilb, [(48 - (ilb.get_width())), 64])
            screen.blit(flb, [(104 - (flb.get_width())), 64])
            screen.blit(pygame.transform.flip(asprs[avars[3][5]][spr], 1, 0), [152, 98])
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                    if (104 < mp[1] < 136) and (mp[0] < 112) and play and end == 0:
                        pygame.mixer.stop()
                        sound[2].play()
                        a = cuo
                        cuo = mp[0] > 55
