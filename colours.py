...
def game(avars, asprs, screen):
    ...
    def scol():
        ...
        for i in range(5):
            a = randint(0, 7)
            if len(c) % 2 == 1:
                while c[len(c) - 1] in range((a - 2), (a + 3)): a = randint(0, 7)
            c.append(a)
        o = [0, 1, 2]
        shuffle(o)
        cl = ((c[(o[0] * 2)] * 32), (c[(o[1] * 2)] * 32), (c[(o[2] * 2)] * 32))
        d = [(cl[0] - (c[((o[0] * 2) + (o[0] < 2))] * 32)), (cl[1] - (c[((o[1] * 2) + (o[1] < 2))] * 32)), (cl[2] - (c[((o[2] * 2) + (o[2] < 2))] * 32))]
        i = pygame.image.load("Sprites/Misc/bg/coloursc.png")
        i.set_palette_at(1, cl)
        i.convert()
        clrs.append(i)
        for n in range(1, 16):
            i = pygame.image.load("Sprites/Misc/bg/coloursc.png")
            dc = [((d[o.index(0)] // 4) * (n % 4)), ((d[o.index(1)] // 4) * (n // 4)), 0]
            dc[0], dc[1], dc[2] = dc[o[0]], dc[o[1]], dc[o[2]]
            ncl = ((cl[0] - dc[0]), (cl[1] - dc[1]), (cl[2] - dc[2]))
            i.set_palette_at(1, ncl)
            i.convert()
            clrs.append(i)
        for i in range(4):
            p = clrs[randint(1, 14)]
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/colourst.png").convert()

    gamebk = pygame.image.load("Sprites/Misc/bg/gamecenter.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/colours.png").convert()
    ...
    while kr:
        if tuts:
            ...
            if end == 0:
                spr = 3 + (anifr % 12 > 5)
                screen.blit(mcol[0], [215, 25])
            else:
                spr = 5 + (2 * (len(mcol) > 0))
            for i in range(16):
                if clrs[i] not in mcol:
                    screen.blit(clrs[i], [(48 + (40 * (i % 4))), (34 + (26 * (i // 4)))])
            screen.blit(asprs[avars[3][5]][spr], [((32 - asprs[avars[3][5]][spr].get_width()) // 2), (24 + ((32 - asprs[avars[3][5]][spr].get_width()) // 2))])
        else:
            screen.blit(gamebk, [0, 0])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                    if (34 < mp[1] < 136) and (40 < mp[0] < 200) and play and end == 0:
                        chc = ((mp[0] - 40) // 40) + (4 * ((mp[1] - 33) // 26))
                        if clrs[chc] == mcol[0]:
                            mcol.pop(0)
                            if len(mcol) == 0:
                                pygame.mixer.stop()
