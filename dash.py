...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/dshtt.png").convert()

    gamebk = pygame.image.load("Sprites/Misc/bg/gamecenter.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/dshtbg.png").convert()

    scrfl = pygame.image.load("Sprites/Misc/bg/dshtscrl.png").convert()

    dsho1 = pygame.image.load("Sprites/Misc/obj/dsht1.png")

    dshot = []
    for i in [0, 1, 2, 3]:
        a = pygame.Surface((8, 8)).convert()
        a.blit(playbk, [-(8 * i), 0])
        a.set_colorkey((0, 255, 255))
        dshot.append(a)
    
    dsho2 = pygame.image.load("Sprites/Misc/obj/dsht2.png").convert()
    ...
    while kr:
        if tuts:
            ...
            if anifr < 12:
                screen.blit(scrfl, [40, 112])
            spr = 11 + (2 * (anifr > 11)) - (4 * (end == 2 and scr < 12))
            if end == 0 and anifr > 11:
                screen.blit(dsho1, [196, 72])
                screen.blit(dshot[ncrd], [196, 72])
            elif end > 0:
                if anifr < 12:
                    screen.blit(dsho1, [80, 72])
                    screen.blit(dshot[ncrd], [80, 72])
                else:
                    if end == 1:
                        screen.blit(dsho2, [16, 72])
                        screen.blit(dsho1, [196, 72])
                        screen.blit(dshot[ncrd], [196, 72])
                    else:
                        if scr < 12:
                            screen.blit(dsho1, [44, 72])
                            screen.blit(dshot[ncrd], [44, 72])
                        else:
                            screen.blit(dsho2, [16, 72])
            screen.blit(pygame.transform.flip(asprs[avars[3][5]][spr], 1, (end == 2 and scr < 12)), [(32 - (16 * ((end == 2 and scr < 12))) + ((32 - asprs[avars[3][5]][spr].get_width()) // 2)), (98 - (16 * (anifr > 11)) + (32 - asprs[avars[3][5]][spr].get_height()))])
        else:
            screen.blit(gamebk, [0, 0])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                if 24 < mp[1] < 136 and end == 1 and play and anifr < 12 and (pb != 2 and pb < 6):
