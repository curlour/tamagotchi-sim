...
def game(avars, asprs, screen):
    ...
    def newvals():
        ...
        fres = res + (choice([-1, 1]) * choice([1, 2, 10, 20]))
        if fres < 0:
            fres = abs(fres)
        ...
        isrc = randint(0, 1)
        stans = fnt.render(str([fres, res][isrc]), 1, (0, 0, 100))
        return(t1, t1tx, t2, t2tx, res, restx, stans, isrc, optp)
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/lclasst.png").convert()
    ...
    while kr:
        if tuts:
            ...
            screen.blit(stans, [(48 - stans.get_width()), 74])
            if end == 0:
                spr = 3 + (anifr % 12 > 5)
                sts = 6
            else:
                screen.blit(restx, [(138 - restx.get_width()), 90])
                spr = [9, (7 - (2 * isrc))][isrc == ycorr]
                sts = spr
        ...
        if anifr < 23:
            ...
            if strt:
                play = True
                strt = False
                t1, t1tx, t2, t2tx, res, restx, stans, isrc, optp = newvals()
