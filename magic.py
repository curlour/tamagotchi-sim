...
def game(avars, asprs, screen):
    ...
    tricks = [[randint(0, 1), randint(0, 1), randint(0, 1), randint(0, 1)],
              [randint(0, 1), randint(0, 1), randint(0, 1), randint(0, 1)]]
    while tricks[0] == tricks[1]:
        tricks[1] = [randint(0, 1), randint(0, 1), randint(0, 1), randint(0, 1)]
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/magict.png").convert()
    tutb = []
    for i in [0, 1]:
        j = pygame.Surface((8, 8)).convert()
        j.blit(tutimg, [-(8 * i), 0])
        tutb.append(j)

    playbk = pygame.image.load("Sprites/Misc/bg/magic.png").convert()

    ready = pygame.image.load("Sprites/Misc/bg/ready.png").convert()
    go = pygame.image.load("Sprites/Misc/bg/go.png").convert()

    magico = []
    m = pygame.image.load("Sprites/Misc/obj/magico.png").convert()
    for i in [0, 1, 2, 3]:
        j = pygame.Surface((24, 24)).convert()
        j.fill((0, 255, 255))
        j.blit(m, [-(24 * (i % 2)), -(24 * (i // 2))])
    ...
    while kr:
        if tuts:
            ...
            for i in range(8):
                screen.blit(tutb[tricks[i > 3][i % 4]], [(120 + (32 * (i % 4))), (72 + (40 * (i > 3)))])
        elif strt:
            ...
            if end == 0:
                spr = 3 + (anifr % 12 > 5)
                screen.blit(pygame.transform.flip(magico[2], 0, (anifr % 12 > 5)), [80, 82])
                if turn == 0:
                    screen.blit(magico[0], [80, 106])
            else:
                if seq == tricks[turn]:
                    spr = 5
                    screen.blit(magico[1 + (2 * turn)], [80, (106 - (24 * turn))])
                else:
                    spr = 7
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                if play and end == 0 and pb in [1, 3]:
                    pygame.mixer.stop()
                    sound[2].play()
                    seq.append(pb // 2)
        ...
        if anifr < (23 + (24 * (play and end == 0))):
            ...
            if strt:
                ...
                if end == 0:
                    ...
                    pygame.mixer.stop()
                    sound[9 + (3 * (seq != tricks[turn]))].play()
                else:
                    if seq == tricks[turn]:
                        scr += 1
                        end = 0
