...
def game(avars, asprs, screen):
    ...
    tutimg = pygame.image.load("Sprites/Misc/bg/bakeryt.png").convert()

    playbk = pygame.image.load("Sprites/Misc/bg/bakery.png").convert()

    ready = pygame.image.load("Sprites/Misc/bg/ready.png").convert()
    go = pygame.image.load("Sprites/Misc/bg/go.png").convert()

    clws = []
    c = pygame.image.load("Sprites/Misc/obj/bkcb.png").convert()
    for i in [0, 1, 2]:
        x = pygame.Surface((24, 104)).convert()
        x.fill((0, 255, 255))
        x.blit(c, [-(24 * i), 0])
        x.set_colorkey((0, 255, 255))
        clws.append(x)

    pastry = [[], []]
    f = ["Milk", "BCheese", "Choco", "Cake", "ChCake", "Donut"]
    for i in range(6):
        y = pygame.image.load("Sprites/Food/" + f[i] + ".png").convert()
        x = pygame.Surface((24, 24)).convert()
        x.fill((0, 255, 255))
        x.blit(y, [0, 0])
        x.set_colorkey((0, 255, 255))
        pastry[i > 2].append(x)
    ...
    while kr:
        if tuts:
            ...
            screen.blit(playbk, [0, 0])
            screen.blit(pastry[1][npas], [108, 76])
            if end < 2:
                spr = 3 + (anifr % 12 > 5)
                if anifr % 6 == 0:
                    pasl.append(randint(0, 2))
                    if len(pasl) > 16:
                        pasl.pop(0)
                    if end == 1:
                        if gpas < 0:
                            ...
                            if clw == 0:
                                ...
                                anifr = 11
                                pygame.mixer.stop()
                                sound[9 + (3 * (gpas != npas))].play()
            else:
                spr = 5 + (2 * (gpas != npas))
            screen.blit(clws[clw], [0, 24])
            for j in range(len(pasl)):
                i = (len(pasl) - 1) - j
                if j < 5:
                    screen.blit(pastry[0][pasl[i]], [(24 * (j + 4)), 40])
                elif j < 7:
                    screen.blit(pastry[0][pasl[i]], [216, (56 + (40 * (j - 5)))])
                elif j == 15:
                    screen.blit(pastry[1][pasl[0]], [0, 104])
            screen.blit(asprs[avars[3][5]][spr], [32, 32])
        else:
            screen.blit(playbk, [0, 0])
            if end == 3:
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pb == 1:
                    ...
                    if (24 < mp[1] < 136) and tuts:
                        ...
                        strt = True
                        anifr = -1
                    if (24 < mp[1] < 136) and play and end == 0 and len(pasl) > 13:
                        pygame.mixer.stop()
                        sound[2].play()
                        end = 1
